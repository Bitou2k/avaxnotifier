const debug = require('debug')('app:jobs:check-delegation-end-soon')
...
const handler = agenda => async job => {
  ...
  const filter = {
    endTime: {
      $gte: moment().unix() - (60 * 60 * 24) - 600,
      $lte: moment().unix() - (60 * 60 * 24),
    },
    $or: [
      { notificationSoonSent: { $exists: false }},
      { notificationSoonSent: false }
    ],
  }
  ...
  debug({ delegators: delegators.length }, delegators[0])
  ...
  for (delegator of delegators) {
    ...
    for (receiver of receivers) {
      ...
      if (count === MESSAGES_PER_JOB) {
        await agenda.schedule('in 2 seconds', 'send delegation end soon message', { messages });
        messages = []
        count = 0
      }
    }
...
module.exports = {
  ...
  job: agenda => agenda.define('check delegation end soon', handler(agenda))
}
